...
本计划用于关闭 GvpA 项目的 M2。当前数据审计和同源划分已经完成，但 PF00741 真实扫描、HMM/结构坐标、独立复核和最终实验登记仍需补齐。全组应先完成 M2 验收，再启动正式 M3 分析。
...
1. 建立 M2 Closure 总 Issue 以及 B、C、D、E 子任务。
2. 为每项任务填写负责人、输入、输出、依赖、审查人和验收标准。
3. 组织 PR #4 中 HMM、结构与 ESM-2 公共模块的非作者审查。
...
1. 审查 `download_hmms.py`、`hmm_coordinates.py` 和 `structure_mapping.py`。
