...
1. 审查并合并 PR #4 中的 ESM-2、PF00741 HMM、坐标和结构映射模块。
2. 所有成员从合并后的最新 `main` 新建任务分支。
3. 每项任务先建立 GitHub Issue，分支和 PR 关联相应 Issue。
...
3. 在冻结数据版本上正式运行 PF00741 HMM 流程并生成 receipt。
...
| 时间 | A | B | C | D | E |
| --- | --- | --- | --- | --- | --- |
| 第 1 天 | 组织 PR #4 审查并建立 Issue | 复核数据版本 | 审查 C 分支可迁移代码 | 环境和小样本冒烟测试 | 建立扰动接口 |
| 第 2 天 | 冻结 M3 验收标准 | 生成输入 manifest | PF00741 正式运行 | discovery 全长 embedding | 设计随机基线 |
