...
> C 分支进展：已完成[来源与标签可行性筛查](reports/M1/C_label_feasibility_v0.1.md)、[B 实现复核](reports/M1/C_B_review.md)，以及数据审计、A3M/MSA、保守区域和 Pfam 注释连接[工具](bioinformatics/README.md)，152 项测试通过。真实输入与后续结果仍缺失，见 [C 的 1–8 号任务及交接清单](reports/C_status.md)。PF01132 实为延伸因子 P 的结构域；气囊蛋白家族 PF00741 也包含 GvpJ 等成员，正式对照和数据定义须据[事实核查](reports/M1/C_fact_check.md)确认。合成验收不代表真实生物学结果。

## 研究背景
本项目计划从 GvpA 氨基酸序列出发，探索功能属性预测与残基级解释，将模型结果与经核实并由团队确认的 Pfam 家族注释、MSA 保守性及可选结构信息比较。C 的背景文献及数据库核查见上述报告，预测目标仍待确认。
...
## 三个研究问题
- RQ1：模型关注的关键位点是否与经核实的 Pfam 注释和 MSA 高保守位点一致？
...
| C | 生物信息负责人 | 文献/数据库、标签证据、MSA、信息熵、Pfam、保守区域及可选结构验证 | 复核 B 数据与 D/E 的生物学合理性 |
...
## 数据获取
B 已在 [B 分支](https://github.com/Lithium9767/MLP/tree/B)提交教师数据清洗与划分脚本及初审报告，尚未交接被 Git 忽略的原始/处理数据，聚类划分尚待实际运行。C 的来源筛查已完成，逐样本来源仍需原始记录。字段见 [数据说明](data/README.md)，标签与许可见 [数据卡](data/dataset_card.md)。原始数据不提交普通 Git；共享位置与校验哈希需随数据交接。
...
## 项目当前状态
M1 骨架基础上，C 已完成来源/标签可行性意见、B 实现复核与可独立运行的数据审计、A3M/MSA、保守性、区域和 Pfam 结果解析工具，并以合成输入验证完整流程。真实数据文件、独立标签、模型/归因结果仍未交付，不能据示例宣称正式生物学发现。各任务完成边界及所需文件见 [C 状态清单](reports/C_status.md)。
